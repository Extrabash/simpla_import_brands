...
{if $message_error != 'no_permission'}
    {if $filename}
		...
		<input type="hidden" name="brand_id" value="{if $brand_id}{$brand_id}{else}0{/if}">
    ...
    {else}
        <form method="post" enctype="multipart/form-data">
            ...
            <div class="form-row ">
                <label for="" class="form-label">Поставщик</label>
    ...
    {capture name = 'scripts'}
        ...
        <script>
            (function() {
                ...
                function do_import(from, brand_id) {
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            for (var key in data.items) {
                                ...
                                var $item = $('<div>', {
                                    ...
                                    html: [$('<a>', {
                                            ...
                                            href: 'index.php?module=ProductAdmin&id=' + data
                                                .items[key].product.id,
                                            target: '_blank'
                                        }),
                                        $('<span>', {
                                            text: data.items[key].variant.name ? ', ' + data
                                                .items[key].variant.name : ''
                                        })
                                    ],
                            ...
                            if (data != false && !data.end) {
                                do_import(data.from, data.brand_id);
                            } else {
                ...
                function counter(countAdded, countUpdated) {
                    if (countAdded > 0)
                        var textAdded = countAdded + plural(countAdded, [' товар добавлен', ' товара добавлено',
                            ' товаров добавлено'
                        ]);

                    if (countUpdated > 0)
                        var textUpdated = countUpdated + plural(countUpdated, [' товар обновлен', ' товара обновлено',
                            ' товаров обновлено'
                        ]);
