...
{	
	...
	public function import()
	{
		...
		$file_brand_id = 0;
		$file_brand_id = $this->request->post('brand_id', 'integer');

		if($file_brand_id == 0 || empty($file_brand_id))
			$file_brand_id = $this->request->get('brand_id', 'integer');
		...
		{ 
			...
			// Импортируем этот товар
	 		if($imported_item = $this->import_item($product, $file_brand_id))
		...
		$result->brand_id = $file_brand_id;   // Импортированные товары
	
		return $result;
	}
	
	// Импорт одного товара $item[column_name] = value;
	private function import_item($item, $file_brand_id)
	{
		...
		if($file_brand_id != 0)
		{
			$product['brand_id'] = $file_brand_id;
		}
		elseif(!empty($item['brand']))
		{
			// Если задан бренд товара
